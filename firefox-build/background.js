// Background Script for GatherTube extension - Firefox Compatible
// Uses browser.* APIs and Manifest V2 background script pattern
...
class GatherTubeBackground {
    ...
    async setDefaultSettings() {
        const result = await browserAPI.storage.local.get(['embedMode', 'closeTabs', 'sortOrder']);
    ...
    async createEmbeddedQueue(videoIds) {
        try {
            ...
            const embedUrl = browserAPI.runtime.getURL('embed_page.html') + 
            ...
            const existingTabs = await browserAPI.tabs.query({
                url: browserAPI.runtime.getURL('embed_page.html') + '*',
...
// Initialize background service worker/script
const gatherTube = new GatherTubeBackground();

// Firefox-specific: Keep background script alive (not needed in Chrome service workers)
